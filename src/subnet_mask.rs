...
pub struct SubnetMask(IPv4, u8);

impl SubnetMask {
	pub fn new(cidr: u8) -> Result<Self, SubnetMaskError> {
		if cidr > 0 && cidr <= 32 {
			Ok(SubnetMask(TABLE[(cidr - 1) as usize], cidr))
		} else {
			Err(SubnetMaskError::CidrOutOfRange)
		}
	}
...
impl TryFrom<&IPv4> for SubnetMask {
	type Error = SubnetMaskError;
	...
	fn try_from(value: &IPv4) -> Result<Self, Self::Error> {
		...
		for expected in TABLE.iter() {
			if value == expected {
				return Ok(SubnetMask::new(cidr).unwrap());
			}

			cidr += 1;
		}

		Err(SubnetMaskError::InvalidSubnetMask)
	}
}
...
mod tests {
	...
	fn assert_error<T>(actual: Result<T, SubnetMaskError>, expected: SubnetMaskError) {
		fn to_ordinal(e: SubnetMaskError) -> usize {
			match e {
				SubnetMaskError::CidrOutOfRange => 1,
				SubnetMaskError::FormatError => 2,
				SubnetMaskError::InvalidSubnetMask => 3,
			}
		}
	...
	fn new_test() {
		for i in 1..=32 {
			let actual = SubnetMask::new(i);
			assert!(actual.is_ok());
			assert_eq!(actual.unwrap().cidr(), i);
		}

		assert_error(SubnetMask::new(33), SubnetMaskError::CidrOutOfRange);
		assert_error(SubnetMask::new(0), SubnetMaskError::CidrOutOfRange);
	}
	...
	fn from_subnet_mask_test() {
		...
		for addr in MASK.iter() {
			let fixture = SubnetMask::try_from(addr).unwrap();

			assert_eq!(cidr, fixture.cidr());
			cidr += 1;
		}

		let fixture = SubnetMask::try_from(&IPv4::try_from("128.1.0.1").unwrap());
		assert_error(fixture, SubnetMaskError::InvalidSubnetMask);
	}

	#[test]
	fn debug_test() {
		for i in 1u8..=32 {
			let fixture = SubnetMask::new(i).unwrap();
	...
	fn network_address_test() {
		let mask = SubnetMask::try_from(&IPv4::try_from("255.255.255.0").unwrap()).unwrap();
